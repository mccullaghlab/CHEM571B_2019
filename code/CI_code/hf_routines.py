...
def perform_hf(basis,atoms):
    ...
    # reorder based on increasing eigenvalues
    idx = e.argsort()
    e = e[idx]
    C = C[:,idx]
    ...
    while deltaE > thresh and iteration < 20:
        ...
        deltaE = abs(hf_energy-previousE)
        previousE = hf_energy
        iteration += 1
    # compute two electron integrals in HF optimized molecular orbitals
    hf_twoE = update_twoE(C,twoE)
    return hf_energy, C, orbitalEnergies, hf_twoE


def perform_hf_scf(basis,atoms,maxIter=20,thresh=1E-4):
    # compute overlap matrix
    S, Sinv = overlap(basis)
    # compute Hcore and twoE that are not dependent on coefficients
    T = kinetic(basis)
